...
Proses rekonsiliasi akuntansi perusahaan kerap menghadapi kendala akibat format laporan mutasi rekening yang beragam dari setiap bank—mulai dari struktur file CSV yang berbeda, dokumen PDF multi-baris, hingga buku kerja Excel bertingkat. Mengonversi data tersebut ke dalam sistem ERP seperti Odoo Accounting sering kali menuntut input manual atau penggunaan aplikasi pihak ketiga yang berisiko terhadap kerahasiaan data.
...
### 2. Ekstraksi Vektor PDF Asli (BCA)
...
### 3. Penggabungan Catatan Transaksi Bertingkat (Multiline)
...
### 4. Rekonsiliasi Saldo & Validasi Integritas Data
...
### 5. Mesin Ekspor Ganda Standar Excel (.xlsx)
...
| Institusi Perbankan | Format Input | Kanal Unduhan | Karakteristik Pemrosesan |
| :--- | :---: | :--- | :--- |
| **Bank Mandiri** | `.csv` | Kopra Mandiri / Mandiri Online | Deteksi otomatis variasi header kolom (`PostDate`, `Remarks`, `Debit`, `Credit`, `Balance`). |
| **Bank Central Asia (BCA)** | `.pdf` | KlikBCA Bisnis / E-Statement Rekening Giro & Tabungan | Ekstraksi langsung dari vektor PDF, rekonstruksi berita transfer multiline, resolusi tahun otomatis. |
| **Bank BRI** | `.csv` | CMS BRI / QLola / BRImo | Normalisasi kolom `TGL_TRAN`, `MUTASI_DEBET`, `MUTASI_KREDIT`, dan `SALDO_AKHIR_MUTASI`. |
| **Bank Syariah Indonesia (BSI)** | `.xlsx` / `.xls` | BSI Net Banking / CMS Giro Wadiah | Ekstraksi metadata rekening, pemilik rekening, dan matriks transaksi multi-kolom. |
...
│       ├── mandiri-parser.js # Mesin parsing CSV Bank Mandiri
│       ├── bri-parser.js     # Mesin parsing CSV Bank BRI
│       └── bsi-parser.js     # Mesin parsing Excel Bank BSI
